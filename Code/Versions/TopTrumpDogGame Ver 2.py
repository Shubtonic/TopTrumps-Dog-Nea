...
def play2(category, turn, compOrPlaye, ranger):
    ...
    if int_cate == 4:  # If category is drool
        if playerCat[int_cate] > computerCat[int_cate]:
            ...
            deck_sort(1, turn,compOrPlaye,ranger,playerCat,computerCat)
        elif playerCat[int_cate] == computerCat[int_cate]:
            print("Both values are the same. The player wins")
            deck_sort(0, turn,compOrPlaye,ranger,playerCat,computerCat)
        else:
...
def deck_sort(value, turn, compOrPlaye, ranger, playerCat, computerCat):
    if value == 0:
        player.append(playerCat) # adds the currents card the player has to the end of the list
        player.append(computerCat) # adds the current card of the comnputer to the end of the player's list
    elif value == 1:
        computer.append(playerCat)
        computer.append(computerCat)
    computer.pop(0) # removes the first element in the computer list
    player.pop(0) # removes the first element in the computer list
    play1(turn + 1,compOrPlaye,ranger)
